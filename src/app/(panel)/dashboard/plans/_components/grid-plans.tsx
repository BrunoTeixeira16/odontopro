    ...
    return (
            <section className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-5">
                {subscriptionPlans.map((plan, index) => (
                    <Card key={plan.id} className={`flex flex-col w-full mx-auto ${index === 2 && "p-0"} ${index === 2 && "border-emerald-500"}`}>
                        {index === 2 && (
                        ...
                        <CardFooter>
                            <SubscriptionButton type={plan.id === "BASIC" ? "BASIC" : plan.id === "INTERMEDIARY" ? "INTERMEDIARY" : "PROFESSIONAL"} />
                        </CardFooter>
                    </Card>
                ))}
            </section>
