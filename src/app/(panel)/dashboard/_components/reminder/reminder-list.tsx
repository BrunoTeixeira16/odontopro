    ...
    return (
        <div className="flex flex-col gap-3">
            <Card>
                ...
                <CardContent>
                    {reminder.length === 0 && (
                        <p className="text-sm lg:text-base text-gray-500">Nenhum lembrete registrado...</p>
                    )}


                    <ScrollArea className="h-[calc(100vh-20rem)] lg:h-[calc(100vh-15rem)] pr-0 w-full flex-1">
                        {reminder.map((item) => (
                            <article className="flex flex-wrap flex-row items-center justify-between py-2 bg-yellow-100 mb-2 px-2 rounded-md" key={item.id}>
