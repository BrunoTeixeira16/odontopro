...
export function ScheduleTimeList({ availableTimeSlots, blockedTimes, clinicTimes, requiredSlots, selectedDate, selectedTime, onSelectTime }: ScheduleTimeListProps) {
    ...
    return (
        <div className="grid grid-cols-3 md:grid-cols-5 gap-2">
            {availableTimeSlots.map((slot) => {
                ...
                return (
                    <Button onClick={ () => slotEnabled && onSelectTime(slot.time) } type="button" variant="outline" key={slot.time} className={cn("h-10 select-none",
