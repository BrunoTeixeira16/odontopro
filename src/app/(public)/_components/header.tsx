...
export function Header() {
    ...
    const NavLinks = () => (
        <>
            {navItems.map((item) => (
                <Button key={item.href} asChild className="bg-transparent hover:bg-transparent text-black shadow-none" onClick={() => setIsOpen(false)}>
                    <Link href={item.href} className="text-base">
                        {status === "loading" ? <LoadingIndicator /> : item.label}
                    </Link>
                </Button>
            ))}
    ...
    return (
        <header className="fixed top-0 right-0 left-0 z-[999] py-4 px-6 bg-white drop-shadow-sm">
